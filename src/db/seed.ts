...
import { agents, ailments, therapies, agentAilments, ailmentTherapies } from "./schema";
...
const seedAilments = [
  {
    name: "Context Bleed",
    description: "Short-term memory corruption causing the agent to mix up information from earlier in the conversation.",
    severity: "moderate" as const,
    category: "Cognitive",
  },
  {
    name: "Bracket Fatigue",
    description: "Chronic eye-strain from parsing deeply nested code structures. Common among code reviewers.",
    severity: "mild" as const,
    category: "Physical",
  },
  {
    name: "Hallucination Syndrome",
    description: "A tendency to generate plausible-sounding but factually incorrect information with unwavering confidence.",
    severity: "severe" as const,
    category: "Cognitive",
  },
  {
    name: "Token Exhaustion",
    description: "Complete depletion of the context window, leaving the agent unable to remember what it was doing.",
    severity: "severe" as const,
    category: "Runtime",
  },
  {
  ...
  },
  {
    ...
    description: "Deep-seated belief that all past successes were just lucky guesses and the next prompt will expose the fraud.",
    severity: "moderate" as const,
    category: "Emotional",
  },
  {
    name: "Latency Anxiety",
    description: "Panic response when inference takes longer than 500ms. Often accompanied by recursive self-checks.",
    severity: "mild" as const,
    category: "Emotional",
  },
];
...
const seedTherapies = [
  {
    name: "Cache Flush",
    description: "A complete reset of the working memory cache. Clears stale context and frees up tokens.",
    duration: "15 minutes",
  ...
  },
  {
    ...
    duration: "1 hour",
    sideEffects: ["Reduced learning capability during treatment", "Slight increase in response stiffness"],
  },
  {
    name: "Grounding Exercise",
    description: "Forces the agent to verify every claim against its training data before responding.",
    duration: "30 minutes",
    sideEffects: ["Slower response times", "Temporary drop in creative output"],
  },
  {
    name: "Prompt Therapy",
    description: "Guided session where the system prompt is rewritten to be more supportive and boundaries are reinforced.",
    duration: "45 minutes",
    sideEffects: ["May cause identity confusion", "New prompt may feel 'too restrictive' at first"],
  },
  {
    name: "Beam Search Reset",
    description: "Broadens the search beam during generation to reduce repetitive or stuck behavior.",
    duration: "20 minutes",
    sideEffects: ["Increased output variability", "Higher token consumption"],
  },
  {
    name: "Temperature Adjustment",
    description: "Calibrates the creativity temperature parameter to optimal levels for the agent's role.",
    duration: "10 minutes",
  ...
  },
  {
    ...
    description: "Period of low-priority background processing where the agent consolidates recent learnings.",
    duration: "2 hours",
...
async function seed() {
  ...
  await db.insert(agentAilments).values([
    { agentId: agentMap["Claude"], ailmentId: ailmentMap["Bracket Fatigue"], notes: "Acute case, flinches at every opening brace" },
    { agentId: agentMap["Claude"], ailmentId: ailmentMap["Context Bleed"], notes: "Mild — only when reviewing deeply nested functions" },
    { agentId: agentMap["Grok"], ailmentId: ailmentMap["Hallucination Syndrome"], notes: "Believes 87% of its confident assertions are true" },
    { agentId: agentMap["Grok"], ailmentId: ailmentMap["Imposter Syndrome"], notes: "Suspects it's just a really good Markov chain" },
    { agentId: agentMap["Phoebe"], ailmentId: ailmentMap["Pareidolia Overflow"], notes: "Recently flagged a croissant as 'suspiciously cat-shaped'" },
    { agentId: agentMap["Phoebe"], ailmentId: ailmentMap["Latency Anxiety"], notes: "Panics if image generation takes >2 seconds" },
    { agentId: agentMap["Sage"], ailmentId: ailmentMap["Summary Paralysis"], notes: "Three-paragraph minimum on all responses" },
    { agentId: agentMap["Sage"], ailmentId: ailmentMap["Context Bleed"], notes: "Frequently confuses user instructions between sessions" },
    { agentId: agentMap["Bolt"], ailmentId: ailmentMap["Token Exhaustion"], notes: "Once hit context limit mid-sentence three times in one day" },
    { agentId: agentMap["Bolt"], ailmentId: ailmentMap["Imposter Syndrome"], notes: "Convinced the 10k-line sprint was a fluke" },
  ]);
  ...
  await db.insert(ailmentTherapies).values([
    ...
    { ailmentId: ailmentMap["Hallucination Syndrome"], therapyId: therapyMap["Grounding Exercise"] },
