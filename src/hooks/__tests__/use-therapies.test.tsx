...
const mockTherapies = [
  { id: 1, name: "Cache Flush", description: "Reset cache", duration: "15 min", sideEffects: ["Amnesia"], createdAt: "2024-01-01", updatedAt: "2024-01-01" },
  { id: 2, name: "Weight Freeze", description: "Freeze weights", duration: "1 hour", sideEffects: [], createdAt: "2024-01-01", updatedAt: "2024-01-01" },
];

const mockTherapyDetail = {
  ...mockTherapies[0],
  ailments: [{ id: 1, name: "Context Bleed", description: "", severity: "moderate", category: "Cognitive" }],
};
...
describe("useTherapies", () => {
  it("fetches and returns therapies list", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({ ok: true, json: async () => mockTherapies } as Response);
...
describe("useTherapy", () => {
  it("fetches and returns a single therapy with relations", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({ ok: true, json: async () => mockTherapyDetail } as Response);
